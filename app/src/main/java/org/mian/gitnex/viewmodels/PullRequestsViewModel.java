...
public class PullRequestsViewModel extends ViewModel {
	...
	private final MutableLiveData<List<Issue>> myPrList = new MutableLiveData<>(new ArrayList<>());
	...
	public LiveData<List<Issue>> getMyPrList() {
		return myPrList;
	}
	...
	public void resetPagination() {
		...
		myPrList.setValue(new ArrayList<>());
	...
	public void searchPullRequests(
			Context ctx,
			String state,
			String labels,
			String milestones,
				...
				new Callback<>() {
					...
					public void onResponse(
						...
						if (response.isSuccessful() && response.body() != null) {
							...
							List<Issue> myPrs = new ArrayList<>();
							...
							List<Issue> currentList =
									isRefresh
											? new ArrayList<>()
											: new ArrayList<>(
													Objects.requireNonNull(myPrList.getValue()));
							currentList.addAll(myPrs);
							myPrList.setValue(currentList);
							...
							if (isRefresh) prList.setValue(new ArrayList<>());
							errorMessage.setValue("API error: " + response.code());
						}
					}
