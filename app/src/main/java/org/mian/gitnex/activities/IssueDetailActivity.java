...
public class IssueDetailActivity extends BaseActivity
		...
		binding.headerSection.btnEdit.setOnClickListener(
				v -> {
					if (isDataLoaded && viewModel.getIssueData().getValue() != null) {
						...
						CreateIssueBottomSheet.newInstance(repositoryContext, issue)
				...
				v -> {
					...
					boolean isBookmarked =
							BookmarkHelper.isBookmarked(
									this, "issue", owner, repo, null, String.valueOf(issueNumber));
	...
	private void handleMenuAction(String id) {
		...
		switch (id) {
			case "dependencies":
				DependenciesBottomSheet.newInstance(owner, repo, issueNumber)
						.show(getSupportFragmentManager(), "DEPENDENCIES");
				break;
			case "tracked_time":
				TrackedTimeBottomSheet.newInstance(owner, repo, issueNumber)
