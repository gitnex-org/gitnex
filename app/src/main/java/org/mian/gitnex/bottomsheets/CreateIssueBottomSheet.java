...
public class CreateIssueBottomSheet extends BottomSheetDialogFragment {
	...
	public static CreateIssueBottomSheet newInstance(
		...
		CreateIssueBottomSheet fragment = new CreateIssueBottomSheet();
	...
	private void setupUI() {
		...
		binding.btnBookmark.setIconTint(
				ColorStateList.valueOf(
						isBookmarked
								? MaterialColors.getColor(requireContext(), R.attr.colorError, 0)
								: MaterialColors.getColor(requireContext(), R.attr.iconsColor, 0)));
		...
		binding.btnBookmark.setOnClickListener(
				v -> {
					...
					BookmarkHelper.toggleBookmark(
							...
							null,
							null,
							getString(R.string.create_issue) + " · " + repoContext.getFullName(),
					...
					boolean nowBookmarked = !isCurrentlyBookmarked;
					binding.btnBookmark.setIconResource(
							nowBookmarked
							...
							ColorStateList.valueOf(
									nowBookmarked
											? MaterialColors.getColor(
													requireContext(), R.attr.colorError, 0)
											: MaterialColors.getColor(
													requireContext(), R.attr.colorPrimary, 0)));
				});
	...
	@NonNull private EditIssueOption getEditIssueOption(String title, String description) {
		EditIssueOption editIssueJson = new EditIssueOption();
		editIssueJson.setTitle(title);
		editIssueJson.setBody(description);

		if (selectedMilestoneId != null) {
			editIssueJson.setMilestone(selectedMilestoneId);
		}

		if (!selectedAssignees.isEmpty()) {
			editIssueJson.setAssignees(new ArrayList<>(selectedAssignees));
		}

		Date dueDate = getDueDateForApi();
		if (dueDate != null) {
			editIssueJson.setDueDate(dueDate);
		}

		return editIssueJson;
	}

	private void handleIssueSuccess(long issueNumber) {
		if (attachmentManager != null && attachmentManager.getAttachmentCount() > 0) {
			attachmentsViewModel.uploadAttachments(
					requireContext(), repoContext.getOwner(), repoContext.getName(), issueNumber);
		} else {
			String successMsg =
					getString(
							issueToEdit != null
	...
	private boolean hasLabelsChanged() {
		...
		return !selectedLabels.equals(originalLabelNames);
	}
	...
	private void observeViewModel() {
		...
		viewModel
				...
				.observe(
						...
						issue -> {
							if (issue != null) {
								handleIssueSuccess(issue.getNumber());
							}
						});
				...
				.observe(
						...
						isUpdating -> {
							if (issueToEdit != null) {
								...
								binding.btnCreate.setEnabled(!isUpdating);
								binding.btnCreate.setText(
		...
		viewModel
				...
				.observe(
						getViewLifecycleOwner(),
						issue -> {
							if (issue != null && issueToEdit != null) {
								if (hasLabelsChanged()) {
									viewModel.updateIssueLabels(
											requireContext(),
											repoContext.getOwner(),
											repoContext.getName(),
											issue.getNumber(),
		...
		viewModel.getIsUpdatingLabels().observe(getViewLifecycleOwner(), isUpdating -> {});
				...
				.observe(
						...
						error -> {
							if (error != null && !error.isEmpty()) {
								...
								viewModel.clearLabelsUpdateError();
							}
						});
							...
							if (complete != null && complete) {
								String successMsg =
										getString(
												issueToEdit != null
														? R.string.editIssueSuccessMessage
														: R.string.issueCreatedWithAttachments);
								Toasty.show(requireContext(), successMsg);
								dismiss();
							}
						});
	...
	private void openLabelPicker() {
		LabelPickerBottomSheet labelPicker =
				LabelPickerBottomSheet.newInstance(repoContext, new ArrayList<>(selectedLabels));
	...
	private void openMilestonePicker() {
		...
		MilestonePickerBottomSheet milestonePicker =
				MilestonePickerBottomSheet.newInstance(repoContext, current);
	...
	private void openAssigneesPicker() {
		AssigneesPickerBottomSheet assigneesPicker =
				AssigneesPickerBottomSheet.newInstance(
